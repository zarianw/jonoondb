...
TEST(Database, Ctor_CreateIfMissing) {
  Database db(g_TestRootDirectory, "Database_Ctor_CreateIfMissing",
...
TEST(Database, CreateCollection_InvalidSchema) {
  ...
  ASSERT_THROW(db.CreateCollection("CollectionName", SchemaType::FLAT_BUFFERS,
                                   "Schema IDL", indexes),
...
TEST(Database, CreateCollection_DuplicateIndex) {
  ...
  std::vector<IndexInfo> indexes;
  indexes.push_back(
      IndexInfo("index1", IndexType::INVERTED_COMPRESSED_BITMAP, "id", true));
  indexes.push_back(IndexInfo("index1", IndexType::INVERTED_COMPRESSED_BITMAP,
                              "user.name", true));
  ASSERT_THROW(db.CreateCollection(collectionName, SchemaType::FLAT_BUFFERS,
                                   schema, indexes),
  ...
  indexes.pop_back();
  ASSERT_NO_THROW(db.CreateCollection(collectionName, SchemaType::FLAT_BUFFERS,
                                      schema, indexes));
}
...
TEST(Database, Insert_NoIndex) {
  ...
  Buffer documentData =
      TestUtils::TestUtils::GetTweetObject(1, 1, &name, &text, 2.0, &binData);
...
TEST(Database, Insert_SingleIndex) {
  ...
  db.CreateCollection(collectionName.c_str(), SchemaType::FLAT_BUFFERS, schema,
                      indexes);
  std::string name = "Zarian";
  std::string text = "Say hello to my little friend!";
  std::string binData = "some_data";
  Buffer documentData =
      TestUtils::GetTweetObject(1, 1, &name, &text, 2.0, &binData);
...
void Execute_Insert_AllIndexTypes_Test(const std::string& dbName,
  ...
  Buffer documentData = TestUtils::GetAllFieldTypeObjectBuffer(
      1, 2, true, 4, 5, 6, 7, 8.0f, 9, 10.0, "test", "test1", "test2",
      nullifyNestedField);
  db.Insert(collectionName, documentData);
}
...
TEST(Database, Insert_AllIndexTypes_EWAHCB) {
  ...
  Execute_Insert_AllIndexTypes_Test(
      dbName, IndexType::INVERTED_COMPRESSED_BITMAP, false);
}
...
TEST(Database, Insert_AllIndexTypes_EWAHCB_NestedNull) {
  ...
  Execute_Insert_AllIndexTypes_Test(
      dbName, IndexType::INVERTED_COMPRESSED_BITMAP, true);
}
...
TEST(Database, ExecuteSelect_MissingCollection) {
  ...
  ASSERT_ANY_THROW(ResultSet rs = db.ExecuteSelect(
                       "select * from missingTable where text = 'hello'"));
}
...
TEST(Database, ExecuteSelect_NonEmptyDB_SingleIndex) {
  ...
  string filePath = GetSchemaFilePath("tweet.bfbs");
  string schema = File::Read(filePath);
  std::vector<IndexInfo> indexes{IndexInfo(
      "IndexName1", IndexType::INVERTED_COMPRESSED_BITMAP, "user.name", true)};
  db.CreateCollection(collectionName, SchemaType::FLAT_BUFFERS, schema,
                      indexes);
...
TEST(Database, ExecuteSelect_Testing) {
  ...
  ResultSet rs = db.ExecuteSelect(
      "SELECT id, text, [user.id], [user.name], rating, binData FROM tweet "
      "WHERE id = 1;");
  while (rs.Next()) {
  ...
  rows = 0;
  rs = db.ExecuteSelect(
      "SELECT id, text, [user.id], [user.name] FROM tweet WHERE [user.name] = "
      "'Zarian' AND text = 'hello'");
  while (rs.Next()) {
    ++rows;
  }
  ASSERT_EQ(rows, 0);
  ...
  rs = db.ExecuteSelect(
      "SELECT id, text, [user.id], [user.name] FROM tweet WHERE [user.name] = "
      "'Zarian' AND text = 'Say hello to my little friend!'");
  while (rs.Next()) {
    ASSERT_EQ(rs.GetInteger(rs.GetColumnIndex("id")), 1);
    ASSERT_STREQ(rs.GetString(rs.GetColumnIndex("text")).str(),
                 "Say hello to my little friend!");
...
void ExecuteCtor_ReopenTest(std::string& dbName, bool enableCompression,
...

  {
    ...
    db.CreateCollection(collectionName1, SchemaType::FLAT_BUFFERS, schema,
                        indexes);
    db.CreateCollection(collectionName2, SchemaType::FLAT_BUFFERS, schema,
                        indexes);
  ...
  std::string sqlStmt =
      "SELECT id, text, [user.id], [user.name], rating, binData FROM " +
      collectionName1 + ";";
  ...
  while (rs.Next()) {
    ...
    ASSERT_STREQ(rs.GetString(rs.GetColumnIndex("user.name")).str(),
                 name.c_str());
    ASSERT_DOUBLE_EQ(rs.GetInteger(rs.GetColumnIndex("rating")),
                     (double)rowCnt);
    std::string binData = "some_data_" + std::to_string(rowCnt);
    auto blob = rs.GetBlob(rs.GetColumnIndex("binData"));
    ASSERT_EQ(blob.GetLength(), binData.size());
  ...
  sqlStmt = "SELECT id, text, [user.id], [user.name], rating, binData FROM " +
            collectionName2 + ";";
...
TEST(Database, ExecuteSelect_Indexed_LessThanInteger) {
  Database db(g_TestRootDirectory, "ExecuteSelect_LessThanInteger",
              TestUtils::GetDefaultDBOptions());
  string filePath = GetSchemaFilePath("tweet.bfbs");
  string schema = File::Read(filePath);
  std::vector<IndexInfo> indexes{IndexInfo(
      "IndexName1", IndexType::INVERTED_COMPRESSED_BITMAP, "id", true)};
...
TEST(Database, ExecuteSelect_VectorIndexer) {
  Database db(g_TestRootDirectory, "ExecuteSelect_VectorIndexer",
              TestUtils::GetDefaultDBOptions());
  string filePath = GetSchemaFilePath("tweet.bfbs");
  string schema = File::Read(filePath);
  std::vector<IndexInfo> indexes{
      IndexInfo("IndexName1", IndexType::VECTOR, "id", true),
  ...
  rowCnt = 0;
  rs = db.ExecuteSelect(
      "SELECT id, text, [user.id], [user.name], rating FROM tweet WHERE rating "
      "< 5.0;");
  while (rs.Next()) {
    ASSERT_EQ(rs.GetInteger(rs.GetColumnIndex("id")), rowCnt);
    ASSERT_EQ(rs.GetDouble(rs.GetColumnIndex("rating")), double(rowCnt));
...
void Execute_ExecuteSelect_LT_LTE_Test(Database& db,
                                       vector<IndexInfo>& indexes) {
  string filePath = GetSchemaFilePath("all_field_type.bfbs");
  string schema = File::Read(filePath);
  db.CreateCollection("all_field_collection", SchemaType::FLAT_BUFFERS, schema,
                      indexes);

  std::vector<Buffer> documents;
  for (size_t i = 0; i < 10; i++) {
    std::string str = std::to_string(i);
    documents.push_back(TestUtils::GetAllFieldTypeObjectBuffer(
  ...
  ExecuteAndValidateResultset(db, "[nestedField.field12]", "<", "x'30'",
                              0);  // x'30' = 0
  ExecuteAndValidateResultset(db, "[nestedField.field12]", "<", "x'3939'",
                              10);  // x'3939' = 99
  ExecuteAndValidateResultset(db, "[nestedField.field12]", "<", "x'35'",
                              5);  // x'35' = 5

  ExecuteAndValidateResultset(db, "[nestedField.field13]", "<", "x'30'",
                              0);  // x'30' = 0
  ExecuteAndValidateResultset(db, "[nestedField.field13]", "<", "x'3939'",
                              10);  // x'3939' = 99
  ExecuteAndValidateResultset(db, "[nestedField.field13]", "<", "x'35'",
                              5);  // x'35' = 5
  ...
  ExecuteAndValidateResultset(db, "[nestedField.field12]", "<=", "x'30'",
                              1);  // x'30' = 0
  ExecuteAndValidateResultset(db, "[nestedField.field12]", "<=", "x'39'",
                              10);  // x'39' = 9
  ExecuteAndValidateResultset(db, "[nestedField.field12]", "<=", "x'35'",
                              6);  // x'35' = 5
  ...
  ExecuteAndValidateResultset(db, "[nestedField.field13]", "<=", "x'30'",
                              1);  // x'30' = 0
  ExecuteAndValidateResultset(db, "[nestedField.field13]", "<=", "x'39'",
                              10);  // x'39' = 9
  ExecuteAndValidateResultset(db, "[nestedField.field13]", "<=", "x'35'",
                              6);  // x'35' = 5
}
...
TEST(Database, ExecuteSelect_LT_LTE) {
  Database db(g_TestRootDirectory, "ExecuteSelect_LT_LTE",
              TestUtils::GetDefaultDBOptions());
  std::vector<IndexInfo> indexes{};
...
void Execute_ExecuteSelect_GT_GTE_Test(Database& db,
  ...
  ExecuteAndValidateResultset(db, "field12", ">", "x'2f'",
                              10);  // x'2f' = / (which is less than 0)
  ExecuteAndValidateResultset(db, "field12", ">", "x'3939'",
                              0);                               // x'3939' = 99
  ExecuteAndValidateResultset(db, "field12", ">", "x'35'", 4);  // x'35' = 5

  ExecuteAndValidateResultset(db, "field13", ">", "x'2f'",
                              10);  // x'2f' = / (which is less than 0)
  ExecuteAndValidateResultset(db, "field13", ">", "x'3939'",
                              0);                               // x'3939' = 99
  ...
  ExecuteAndValidateResultset(db, "[nestedField.field12]", ">", "x'2f'",
                              10);  // x'2f' = / (which is less than 0)
  ExecuteAndValidateResultset(db, "[nestedField.field12]", ">", "x'3939'",
                              0);  // x'3939' = 99
  ExecuteAndValidateResultset(db, "[nestedField.field12]", ">", "x'35'",
  ...
  ExecuteAndValidateResultset(db, "[nestedField.field13]", ">", "x'2f'",
                              10);  // x'2f' = / (which is less than 0)
  ExecuteAndValidateResultset(db, "[nestedField.field13]", ">", "x'3939'",
                              0);  // x'3939' = 99
  ExecuteAndValidateResultset(db, "[nestedField.field13]", ">", "x'35'",
...
TEST(Database, ExecuteSelect_VECTORIndexed_DoubleExpression) {
  ...
  string schema = File::Read(filePath);
  std::vector<IndexInfo> indexes{
      IndexInfo("IndexName1", IndexType::VECTOR, "rating", true)};
  db.CreateCollection("tweet", SchemaType::FLAT_BUFFERS, schema, indexes);
  ...
  for (size_t i = 0; i < 10; i++) {
    ...
    std::string text = "hello_" + std::to_string(i);
    std::string binData = "some_data_" + std::to_string(i);
    documents.push_back(TestUtils::GetTweetObject(i, i, &name, &text,
                                                  (double)i / 100.0, &binData));
  }
  db.MultiInsert("tweet", documents);

  int rowCnt = 0;
  auto rs = db.ExecuteSelect(
      "SELECT id, text, [user.id], [user.name], rating "
...
TEST(Database, ExecuteSelect_DoubleExpression) {
  Database db(g_TestRootDirectory, "ExecuteSelect_DoubleExpression",
              TestUtils::GetDefaultDBOptions());
  string filePath = GetSchemaFilePath("tweet.bfbs");
  string schema = File::Read(filePath);
  std::vector<IndexInfo> indexes;
...
TEST(Database, ExecuteSelect_EWAHIndexed_String_GTE) {
  Database db(g_TestRootDirectory, "ExecuteSelect_EWAHIndexed_String_GTE",
              TestUtils::GetDefaultDBOptions());
  string filePath = GetSchemaFilePath("tweet.bfbs");
  string schema = File::Read(filePath);
  std::vector<IndexInfo> indexes{IndexInfo(
...
void ValidateTweetResultSet(Database& db, int lowerCount, int upperCount,
  ...
  std::string sql =
      "SELECT id, text, [user.id], [user.name], rating FROM tweet WHERE ";
  sql.append(lowerColName)
      .append(" ")
      .append(lowerOp)
      .append(" ")
      .append(lowerColVal)
...
TEST(Database, ExecuteSelect_EWAHIndexed_Range) {
  Database db(g_TestRootDirectory, "ExecuteSelect_EWAHIndexed_Range",
              TestUtils::GetDefaultDBOptions());
  string filePath = GetSchemaFilePath("tweet.bfbs");
  string schema = File::Read(filePath);
  std::vector<IndexInfo> indexes{
      IndexInfo("IndexName1", IndexType::INVERTED_COMPRESSED_BITMAP, "id",
                true),
      IndexInfo("IndexName2", IndexType::INVERTED_COMPRESSED_BITMAP, "rating",
                true),
      IndexInfo("IndexName3", IndexType::INVERTED_COMPRESSED_BITMAP, "user.id",
                true),
      IndexInfo("IndexName4", IndexType::INVERTED_COMPRESSED_BITMAP,
                "user.name", true)};
  db.CreateCollection("tweet", SchemaType::FLAT_BUFFERS, schema, indexes);
  ...
  for (size_t i = 0; i < 10; i++) {
    ...
    std::string text = "hello_" + std::to_string(i);
    std::string binData = "some_data_" + std::to_string(i);
    documents.push_back(TestUtils::GetTweetObject(
        i, i, &name, &text, static_cast<double>(i), &binData));
  }
  db.MultiInsert("tweet", documents);

  ValidateTweetResultSet(db, 3, 8, "[user.name]", ">=", "'zarian_3'",
                         "[user.name]", "<=", "'zarian_7'");
  ValidateTweetResultSet(db, 3, 7, "[user.name]", ">=", "'zarian_3'",
                         "[user.name]", "<", "'zarian_7'");
  ValidateTweetResultSet(db, 4, 8, "[user.name]", ">", "'zarian_3'",
                         "[user.name]", "<=", "'zarian_7'");
  ValidateTweetResultSet(db, 4, 7, "[user.name]", ">", "'zarian_3'",
                         "[user.name]", "<", "'zarian_7'");
  ...
  ValidateTweetResultSet(db, 3, 8, "[user.id]", ">=", "3", "[user.id]",
                         "<=", "7");
  ValidateTweetResultSet(db, 3, 7, "[user.id]", ">=", "3", "[user.id]", "<",
                         "7");
  ValidateTweetResultSet(db, 4, 8, "[user.id]", ">", "3", "[user.id]",
                         "<=", "7");
  ValidateTweetResultSet(db, 4, 7, "[user.id]", ">", "3", "[user.id]", "<",
                         "7");

  ValidateTweetResultSet(db, 3, 8, "rating", ">=", "3.0", "rating",
                         "<=", "7.0");
...
TEST(Database, ExecuteSelect_VECTORIndexed_Range) {
  Database db(g_TestRootDirectory, "ExecuteSelect_VECTORIndexed_Range",
              TestUtils::GetDefaultDBOptions());
  string filePath = GetSchemaFilePath("tweet.bfbs");
  string schema = File::Read(filePath);
...
TEST(Database, ExecuteSelect_NullStrFields) {
  Database db(g_TestRootDirectory, "ExecuteSelect_NullStrFields",
              TestUtils::GetDefaultDBOptions());
  string filePath = GetSchemaFilePath("tweet.bfbs");
  string schema = File::Read(filePath);
  std::vector<IndexInfo> indexes;
  ...
  for (size_t i = 0; i < 10; i++) {
    ...
    if (i % 2 == 0) {
      documents.push_back(TestUtils::GetTweetObject(
          i, i, &name, &text, (double)i / 100.0, &binData));
    } else {
      documents.push_back(TestUtils::GetTweetObject(
          i, i, nullptr, nullptr, (double)i / 100.0, nullptr));
    }
  }
  db.MultiInsert("tweet", documents);
...
TEST(Database, ExecuteSelect_NullStrFields_VectorIndexed) {
  Database db(g_TestRootDirectory, "ExecuteSelect_NullStrFields_Indexed",
              TestUtils::GetDefaultDBOptions());
  string filePath = GetSchemaFilePath("tweet.bfbs");
  string schema = File::Read(filePath);
  std::vector<IndexInfo> indexes{
      IndexInfo("IndexName1", IndexType::VECTOR, "text", true),
      IndexInfo("IndexName4", IndexType::VECTOR, "user.name", true)};
  ...
  for (size_t i = 0; i < 10; i++) {
    ...
    if (i % 2 == 0) {
      ...
      documents.push_back(TestUtils::GetTweetObject(
          i, i, nullptr, nullptr, (double)i / 100.0, &binData));
    }
  }
  db.MultiInsert("tweet", documents);
...
TEST(Database, ExecuteSelect_NullStrFields_EWAHIndexed) {
  Database db(g_TestRootDirectory, "ExecuteSelect_NullStrFields_EWAHIndexed",
              TestUtils::GetDefaultDBOptions());
  string filePath = GetSchemaFilePath("tweet.bfbs");
  string schema = File::Read(filePath);
  std::vector<IndexInfo> indexes{
      IndexInfo("IndexName1", IndexType::INVERTED_COMPRESSED_BITMAP, "text",
                true),
      IndexInfo("IndexName4", IndexType::INVERTED_COMPRESSED_BITMAP,
                "user.name", true)};
...
TEST(Database, NullNestedField) {
  ...
  Buffer documentData = TestUtils::GetAllFieldTypeObjectBuffer(
      1, 2, true, 4, 5, 6, 7, 8.0f, 9, 10.0, "test", "test1", "test2", true);
  db.Insert(collectionName, documentData);

  auto rs = db.ExecuteSelect(
      "select field1, \"nestedField.field1\", "
...
TEST(Database, Insert_Invalid) {
  ...
  ASSERT_THROW(db.Insert(collectionName, documentData), JonoonDBException);
}
