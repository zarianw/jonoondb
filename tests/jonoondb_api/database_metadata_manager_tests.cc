...
TEST(DatabaseMetadataManager, Ctor_New) {
  ...
  ASSERT_NO_THROW({
    std::unique_ptr<DatabaseMetadataManager> databaseMetadataManager =
        std::make_unique<DatabaseMetadataManager>(g_TestRootDirectory, dbName,
                                                  true);
  });
}
...
TEST(DatabaseMetadataManager, AddCollection_NewSingleIndex) {
  ...
  ASSERT_NO_THROW({
    ...
    IndexInfoImpl indexInfo("MyIndex", IndexType::INVERTED_COMPRESSED_BITMAP,
                            "MyColumn", true);
    std::vector<IndexInfoImpl*> vec;
    vec.push_back(&indexInfo);
    databaseMetadataManager->AddCollection("MyCollection",
                                           SchemaType::FLAT_BUFFERS, "", vec);
  });
}
...
TEST(DatabaseMetadataManager, AddCollection_DiffCollSameIndexName) {
  ...
  ASSERT_NO_THROW({
    databaseMetadataManager->AddCollection(
        "MyCollection", SchemaType::FLAT_BUFFERS, "", indexes);
    databaseMetadataManager->AddCollection(
        "MyCollection2", SchemaType::FLAT_BUFFERS, "", indexes);
  });
}
