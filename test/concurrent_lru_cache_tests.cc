...
TEST(ConcurrentLRUCache, AddAndFind)
{
...

  {
    ...
    for (int i = 0; i < arraySize; i++)
    {
      ...
      lruCache.Add(cacheObjectPtrs[i]->Data, shared_ptr<CacheObject>(cacheObjectPtrs[i]), true);
    }
...
TEST(ConcurrentLRUCache, PerformEviction_EvictableFlag)
{
...

  {
    ...
    for (int i = 0; i < arraySize; i++)
    {
      ...
      if ((i + 1) % 5 == 0)
      {
        lruCache.Add(cacheObjectPtrs[i]->Data, shared_ptr<CacheObject>(cacheObjectPtrs[i]), false);
      }
      else
      {
        lruCache.Add(cacheObjectPtrs[i]->Data, shared_ptr<CacheObject>(cacheObjectPtrs[i]), true);
