...
TEST(Buffer, Buffer_MoveConstructor)
{
  ...
  ASSERT_TRUE(buffer6.GetDataForWrite() != nullptr && buffer6.GetDataForWrite() == str.data());

  ASSERT_TRUE(buffer6.GetCapacity() == str.length());
  ASSERT_TRUE(buffer6.GetLength() == str.length());

  ASSERT_TRUE(memcmp(buffer6.GetData(), str.data(), buffer6.GetCapacity()) == 0);
  ASSERT_TRUE(memcmp(buffer6.GetDataForWrite(), str.data(), buffer6.GetCapacity()) == 0);
}
...
TEST(Buffer, Buffer_Assign)
{
  ...
  Buffer buffer1;
  ASSERT_TRUE(buffer1.Assign(const_cast<char*>(str.data()), str.length(), str.capacity(), nullptr).InvalidArgument());
  ...
  //2. Valid condition.
  Buffer buffer2;
  ASSERT_TRUE(buffer2.Assign(const_cast<char*>(str.data()), str.length(), str.capacity(), StandardDeleteNoOp).OK());
  ...
  ASSERT_TRUE(memcmp(buffer2.GetData(), str.data(), buffer2.GetCapacity()) == 0);
  ASSERT_TRUE(memcmp(buffer2.GetDataForWrite(), str.data(), buffer2.GetCapacity()) == 0);


  //3. This is an error condition because we are specifying length < capacity
  Buffer buffer3;
  ASSERT_TRUE(buffer3.Assign(const_cast<char*>(str.data()), 20, 10, StandardDeleteNoOp).InvalidArgument());
  ...
  Buffer buffer4;
  ASSERT_TRUE(buffer4.Assign(const_cast<char*>(str.data()), 0, 0, StandardDeleteNoOp).OK());
...
TEST(Buffer, Buffer_Copy_Exact)
{
  ...
  Buffer buffer1;
  ASSERT_TRUE(buffer1.Copy(const_cast<char*>(str.data()), str.length(), str.capacity()).OK());

  ASSERT_TRUE(buffer1.GetData() != nullptr && buffer1.GetData() != str.data());
  ASSERT_TRUE(buffer1.GetDataForWrite() != nullptr && buffer1.GetDataForWrite() != str.data());

  ASSERT_TRUE(buffer1.GetCapacity() == str.capacity());
  ASSERT_TRUE(buffer1.GetLength() == str.length());

  ASSERT_TRUE(memcmp(buffer1.GetData(), str.data(), buffer1.GetCapacity()) == 0);
  ASSERT_TRUE(memcmp(buffer1.GetDataForWrite(), str.data(), buffer1.GetCapacity()) == 0);

  //2. Invalid Arguments
  Buffer buffer2;
  ASSERT_TRUE(buffer2.Copy(nullptr, 2, 2).InvalidArgument());
  ASSERT_TRUE(buffer2.Copy(const_cast<char*>(str.data()), 4, 2).InvalidArgument());

  //3. A valid buffer should be reset when capacity is 0
  Buffer buffer3;
  ASSERT_TRUE(buffer3.Copy(const_cast<char*>(str.data()), str.length(), str.capacity()).OK());
  ASSERT_TRUE(buffer3.GetData() != nullptr && buffer3.GetData() != str.data());
  ASSERT_TRUE(buffer3.GetDataForWrite() != nullptr && buffer3.GetDataForWrite() != str.data());
  ...
  Buffer buffer4;
  ASSERT_TRUE(buffer4.Copy(const_cast<char*>(str.data()), str.length(), str.capacity()).OK());
  const char* internalDataPtr = buffer4.GetData();
  size_t capacity = buffer4.GetCapacity();

  ASSERT_TRUE(buffer4.Copy(const_cast<char*>(str.data()), str.length(), str.capacity()).OK());

  ASSERT_TRUE(buffer4.GetData() != nullptr && buffer4.GetData() == internalDataPtr);
  ASSERT_TRUE(buffer4.GetDataForWrite() != nullptr && buffer4.GetDataForWrite() == internalDataPtr);
...
TEST(Buffer, Buffer_Copy_Bytes)
{
  ...
  Buffer buffer1;
  ASSERT_TRUE(buffer1.Copy(const_cast<char*>(str.data()), str.length(), originalCapacity).OK());
  ...
  ASSERT_TRUE(buffer1.Copy(const_cast<char*>(strLong.data()), strLong.length()).OK());
...
TEST(Buffer, Buffer_OutOfMemory)
{
  ...
  Buffer buffer1;
  ASSERT_TRUE(buffer1.Copy(const_cast<char*>(str.data()), str.length(), ((size_t)-1)).OutOfMemoryError()); // 1 TB
