...
Status FlatbuffersDocument::GetDocumentValue(const char* fieldName, Document*& val) const {
  ...
  if (fbDoc == nullptr) {
    ...
    string errorMsg = "Argument val cannot be casted to underlying document implementation i.e. FlatbuffersDocument. Make sure you are creating the val by calling AllocateDocument call.";
...
Status FlatbuffersDocument::AllocateDocument(Document*& doc) const {
  doc = new FlatbuffersDocument(m_parser, move(unique_ptr<DynamicTableReader>(new DynamicTableReader())));
  return Status();
}

void FlatbuffersDocument::FreeDocument(Document*& doc) const {
  delete doc;
  doc = nullptr;
}
