...
FlatbuffersDocument::FlatbuffersDocument(
    ...
    : m_fbDcumentSchema(fbDocumentSchema),
      m_buffer(buffer),
      m_obj(const_cast<reflection::Object*>(
          reflection::GetSchema(fbDocumentSchema->GetSchemaText().c_str())
              ->root_table())),
      m_table(flatbuffers::GetAnyRoot((uint8_t*)(buffer->GetData()))) {}

std::string FlatbuffersDocument::GetStringValue(
    const std::string& fieldName) const {
  auto fieldDef = m_obj->fields()->LookupByKey(fieldName.c_str());
  if (fieldDef == nullptr) {
    throw JonoonDBException(GetMissingFieldErrorString(fieldName), __FILE__,
                            __func__, __LINE__);
  }
...
std::int64_t FlatbuffersDocument::GetIntegerValueAsInt64(
    const std::string& fieldName) const {
  auto fieldDef = m_obj->fields()->LookupByKey(fieldName.c_str());
  if (fieldDef == nullptr) {
    throw JonoonDBException(GetMissingFieldErrorString(fieldName), __FILE__,
...
double FlatbuffersDocument::GetFloatingValueAsDouble(
    const std::string& fieldName) const {
  auto fieldDef = m_obj->fields()->LookupByKey(fieldName.c_str());
  if (fieldDef == nullptr) {
    throw JonoonDBException(GetMissingFieldErrorString(fieldName), __FILE__,
...
bool FlatbuffersDocument::TryGetDocumentValue(const std::string& fieldName,
  ...
  auto obj = reflection::GetSchema(m_fbDcumentSchema->GetSchemaText().c_str())
                 ->objects()
                 ->Get(fieldDef->type()->index());
  assert(obj != nullptr);
  ...
  try {
    ...
    fbDoc.SetMembers(m_fbDcumentSchema, m_buffer,
                     const_cast<reflection::Object*>(obj), table, structure);
  } catch (std::bad_cast) {
    // This means that the passed in doc cannot be casted to FlatbuffersDocument
    string errorMsg =
        "Argument val cannot be casted to underlying document "
...
bool VerifyStruct(flatbuffers::Verifier& v,
                  const flatbuffers::Table* parentTable, uint16_t fieldOffset,
                  const reflection::Object* obj, bool isRequired) {
  assert(obj->is_struct());
  auto offset = parentTable->GetOptionalFieldOffset(fieldOffset);
  if (!isRequired) {
    // Check the actual field.
    return !offset ||
           v.Verify(reinterpret_cast<const uint8_t*>(parentTable) + offset,
                    obj->bytesize());
  } else {
    return offset &&
           v.Verify(reinterpret_cast<const uint8_t*>(parentTable) + offset,
...
bool VerifyVectorOfStructs(flatbuffers::Verifier& v,
                           ...
                           uint16_t fieldOffset, const reflection::Object* obj,
                           bool isRequired) {
...
bool FlatbuffersDocument::VerifyVector(
    flatbuffers::Verifier& v, const flatbuffers::Table* table,
  ...
  switch (vecField->type()->element()) {
    ...
    case reflection::BaseType::Obj: {
      auto obj =
          reflection::GetSchema(m_fbDcumentSchema->GetSchemaText().c_str())
              ->objects()
              ->Get(vecField->type()->index());
...
bool FlatbuffersDocument::Verify() const {
  flatbuffers::Verifier v(reinterpret_cast<const uint8_t*>(m_buffer->GetData()),
                          m_buffer->GetLength());
