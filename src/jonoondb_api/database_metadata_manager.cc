...
DatabaseMetadataManager::DatabaseMetadataManager(const char* dbPath, const char* dbName, bool createDBIfMissing)
  : m_dbPath(dbPath), m_dbName(dbName), m_createDBIfMissing(createDBIfMissing), m_metadataDBConnection(nullptr)
{
}
...
Status DatabaseMetadataManager::CreateTables()
{
  // Set DB Pragmas
  int sqliteCode = 0;
  sqliteCode = sqlite3_exec(m_metadataDBConnection, "PRAGMA synchronous = FULL;", 0, 0, 0);
  ...
  string sql = "create table if not exists CollectionSchema(CollectionName text primary key, CollectionSchema text, CollectionSchemaType int)";
  ...
  sql = "create table if not exists CollectionIndex(IndexName text primary key, CollectionName text, IndexInfo blob)";
  ...
  sql = "create table if not exists CollectionDocumentFile(FileKey int primary key, FileName text, FileDataLength int, CollectionName text)";
...
Status DatabaseMetadataManager::Open(const char* dbPath, const char* dbName, bool createDBIfMissing,
  DatabaseMetadataManager*& databaseMetadataManager)
{
  string errorMessage;
...
Status DatabaseMetadataManager::AddCollection(const char* name, int schemaType, const char* schema,
  const IndexInfo indexes[], int indexesLength)
{
  ...
  unique_ptr<sqlite3_stmt, Status(*)(sqlite3_stmt*)> statementGuard(m_insertCollectionSchemaStmt,
    SQLiteUtils::ClearAndResetStatement);

  int sqliteCode = sqlite3_bind_text(
    m_insertCollectionSchemaStmt,
    1,  // Index of wildcard
    name, // CollectionName
...
Status DatabaseMetadataManager::CreateIndex(const char* collectionName, const IndexInfo& indexInfo)
{
  unique_ptr<sqlite3_stmt, Status(*)(sqlite3_stmt*)> statementGuard(m_insertCollectionIndexStmt,
    SQLiteUtils::ClearAndResetStatement);

  int sqliteCode = sqlite3_bind_text(
    m_insertCollectionIndexStmt,
    1,  // Index of wildcard
    indexInfo.GetName(),
    -1, // length of the string is the number of bytes up to the first zero terminator
    SQLITE_STATIC);
