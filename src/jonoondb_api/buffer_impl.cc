...
BufferImpl::BufferImpl()
    : m_buffer(nullptr, StandardDelete), m_length(0), m_capacity(0) {}
...
BufferImpl::BufferImpl(const char* buffer, size_t bufferLengthInBytes,
  ...
  if (buffer == nullptr && bufferLengthInBytes == 0 &&
    ...
    throw InvalidArgumentException(
        "Argument buffer is nullptr whereas bufferLengthInBytes or "
        "bufferCapacityInBytes are greater than 0.",
        __FILE__, __func__, __LINE__);
  } else if (bufferLengthInBytes == 0 || bufferCapacityInBytes == 0) {
    throw InvalidArgumentException(
        "Argument buffer is a valid pointer but bufferLengthInBytes or "
        "bufferCapacityInBytes are 0.",
        __FILE__, __func__, __LINE__);
  } else if (bufferCapacityInBytes < bufferLengthInBytes) {
    // Capacity cannot be less than length
    throw InvalidArgumentException(
        "Argument bufferCapacityInBytes cannot be less than "
        "bufferLengthInBytes.",
        __FILE__, __func__, __LINE__);
  } else {
...
BufferImpl::BufferImpl(char* buffer, size_t bufferLengthInBytes,
  ...
  if (buffer == nullptr && bufferLengthInBytes == 0 &&
      bufferCapacityInBytes == 0) {
    m_buffer = std::unique_ptr<char, void (*)(char*)>(
        nullptr, (customDeleterFunc == nullptr) ? StandardDeleteNoOp
                                                : customDeleterFunc);
    m_length = 0;
    m_capacity = 0;
  } else if (buffer == nullptr) {
    ...
    m_buffer = std::unique_ptr<char, void (*)(char*)>(
        buffer, (customDeleterFunc == nullptr) ? StandardDeleteNoOp
                                               : customDeleterFunc);
    m_length = bufferLengthInBytes;
    m_capacity = bufferCapacityInBytes;
  }
    ...
    if (other.GetData() == nullptr) {
      ...
      if (GetCapacity() != other.GetCapacity()) {
        std::unique_ptr<char, void (*)(char*)> data(
            new char[other.GetCapacity()], StandardDelete);
...
void BufferImpl::Resize(size_t newBufferCapacityInBytes) {
  if (newBufferCapacityInBytes == 0) {
    ...
    std::unique_ptr<char, void (*)(char*)> data(
        new char[newBufferCapacityInBytes], StandardDelete);
    m_buffer = std::move(data);
    m_length = 0;
...
void BufferImpl::SetLength(size_t val) {
  if (GetCapacity() < val) {
    ...
    ss << "Argument val specifying buffer length " << val
       << " cannot be greater than the buffer capacity " << GetCapacity()
       << ".";
...
void BufferImpl::Copy(const char* buffer, size_t bytesToCopy) {
  if (buffer == nullptr) {
    throw InvalidArgumentException("Argument buffer is nullptr.", __FILE__,
                                   __func__, __LINE__);
