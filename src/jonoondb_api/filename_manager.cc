...
FileNameManager::FileNameManager(const std::string& dbPath,
    ...
    : m_collectionName(collectionName), m_db(nullptr, GuardFuncs::SQLite3Close),
      m_getFileNameStatement(nullptr),
      m_getLastFileKeyStatement(nullptr), m_putStatement(nullptr),
  ...
  sqlite3* db = nullptr;
  int sqliteCode = sqlite3_open(pathObj.generic_string().c_str(),
                                &db);  //, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE | SQLITE_OPEN_FULLMUTEX, nullptr);
  m_db.reset(db);
  ...
  sqliteCode = sqlite3_busy_handler(m_db.get(),
    ...
    if (sqliteCode == SQLITE_DONE) {
      ...
      if (createIfMissing) {
        ...
        throw JonoonDBException(
            "Cannot get the current FileInfo because there are no FileInfo records in the database.",
            __FILE__,
            __func__,
            __LINE__);
      }
...

  {
    ...
    if (sqliteCode != SQLITE_ROW) {
      if (sqliteCode == SQLITE_DONE) {
        //This means there are no object file records.			
        throw JonoonDBException(
            "Cannot get the next FileInfo because there are no FileInfo records in the database.",
            __FILE__,
            __func__,
            __LINE__);
      } else {
...
void FileNameManager::UpdateDataFileLength(int fileKey, int64_t length) {
  ...
  std::unique_ptr<sqlite3_stmt, void (*)(sqlite3_stmt*)>
      statementGuard(m_updateStatement, SQLiteUtils::ClearAndResetStatement);
...
void FileNameManager::AddFileRecord(int fileKey, const std::string& fileName) {
  //statement guard will make sure that the statement is cleared and reset when statementGuard object goes out of scope
  std::unique_ptr<sqlite3_stmt, void (*)(sqlite3_stmt*)>
      statementGuard(m_putStatement, SQLiteUtils::ClearAndResetStatement);
