...
FileNameManager::FileNameManager(const std::string& dbPath,
                                 const std::string& dbName,
                                 const std::string& collectionName,
                                 bool createDBIfMissing)
    : m_collectionName(collectionName),
      m_db(nullptr, GuardFuncs::SQLite3Close),
      m_getFileNameStatement(nullptr),
      m_getLastFileKeyStatement(nullptr),
      m_putStatement(nullptr),
  ...
  sqliteCode = sqlite3_busy_handler(
      m_db.get(), SQLiteUtils::SQLiteGenericBusyHandler, nullptr);
  if (sqliteCode != SQLITE_OK) {
    throw SQLException(sqlite3_errstr(sqliteCode), __FILE__, __func__,
                       __LINE__);
  }
  ...
  sqliteCode = sqlite3_prepare_v2(
      m_db.get(),
      "INSERT INTO CollectionDataFile (CollectionName, FileKey, FileName, "
      "FileDataLength) VALUES (?, ?, ?, ?)",  // stmt
  ...
  sqliteCode = sqlite3_prepare_v2(
      m_db.get(),
      "SELECT FileName FROM CollectionDataFile WHERE CollectionName = ? AND "
      "FileKey = ?",  // stmt
      -1,  // If greater than zero, then stmt is read up to the first null
           // terminator
  ...
  sqliteCode = sqlite3_prepare_v2(
      m_db.get(),
      "UPDATE CollectionDataFile SET FileDataLength = ? WHERE CollectionName = "
      "? AND FileKey = ?",  // stmt
      -1,  // If greater than zero, then stmt is read up to the first null
           // terminator
...
void FileNameManager::GetCurrentDataFileInfo(bool createIfMissing,
                                             FileInfo& fileInfo) {
  ...
  std::unique_ptr<sqlite3_stmt, void (*)(sqlite3_stmt*)> statementGuard(
      m_getLastFileKeyStatement, SQLiteUtils::ClearAndResetStatement);

  int sqliteCode = sqlite3_bind_text(m_getLastFileKeyStatement,
                                     1,  // Index of wildcard
    ...
    if (sqliteCode == SQLITE_DONE) {
      ...
      if (createIfMissing) {
        ...
        throw JonoonDBException(
            "Cannot get the current FileInfo because there are no FileInfo "
...
void FileNameManager::GetNextDataFileInfo(FileInfo& fileInfo) {
  ...
  {
    // statement guard will make sure that the statement is cleared and reset
    // when statementGuard object goes out of scope
    std::unique_ptr<sqlite3_stmt, void (*)(sqlite3_stmt*)> statementGuard(
        m_getLastFileKeyStatement, SQLiteUtils::ClearAndResetStatement);
    ...
    if (sqliteCode != SQLITE_ROW) {
      if (sqliteCode == SQLITE_DONE) {
        // This means there are no object file records.
        throw JonoonDBException(
            "Cannot get the next FileInfo because there are no FileInfo "
            "records in the database.",
            __FILE__, __func__, __LINE__);
      } else {
        throw SQLException(sqlite3_errstr(sqliteCode), __FILE__, __func__,
...
void FileNameManager::UpdateDataFileLength(int fileKey, int64_t length) {
  ...
  // statementGuard object goes out of scope
  std::unique_ptr<sqlite3_stmt, void (*)(sqlite3_stmt*)> statementGuard(
      m_updateStatement, SQLiteUtils::ClearAndResetStatement);

  int sqliteCode = sqlite3_bind_int64(m_updateStatement,
                                      1,  // Index of wildcard
                                      length);
  ...
  sqliteCode = sqlite3_bind_text(m_updateStatement,
                                 2,  // Index of wildcard
                                 m_collectionName.c_str(),
                                 -1,  // -1 means go until NULL char
                                 SQLITE_STATIC);
  ...
  sqliteCode = sqlite3_bind_int(m_updateStatement,
                                3,  // Index of wildcard
                                fileKey);
...
void FileNameManager::GetFileInfo(const int fileKey,
  ...
  if (!m_fileInfoMap.Find(fileKey, fileInfo)) {
    ...
    // when statementGuard object goes out of scope
    std::unique_ptr<sqlite3_stmt, void (*)(sqlite3_stmt*)> statementGuard(
        m_getFileNameStatement, SQLiteUtils::ClearAndResetStatement);

    int sqliteCode = sqlite3_bind_text(m_getFileNameStatement,
                                       1,  // Index of wildcard
                                       m_collectionName.c_str(),
                                       -1,  // -1 means go until NULL char
                                       SQLITE_STATIC);
    ...
    sqliteCode = sqlite3_bind_int64(m_getFileNameStatement,
                                    2,  // Index of wildcard
                                    fileKey);
    ...
    if (sqliteCode != SQLITE_ROW) {
      if (sqliteCode == SQLITE_DONE) {
        ...
        // that does not exist in CollectionDataFile table. This is a serious
        // error and should never happen. However the code has to handle this
        // and report the correct error to the user.
        std::ostringstream ss;
    ...
    fileInfo->fileKey = fileKey;
    fileInfo->fileName =
        (const char*)sqlite3_column_text(m_getFileNameStatement, 0);
...
void FileNameManager::AddFileRecord(int fileKey, const std::string& fileName) {
  ...
  // statementGuard object goes out of scope
  std::unique_ptr<sqlite3_stmt, void (*)(sqlite3_stmt*)> statementGuard(
      m_putStatement, SQLiteUtils::ClearAndResetStatement);

  int sqliteCode = sqlite3_bind_text(m_putStatement,
                                     1,  // Index of wildcard
                                     m_collectionName.c_str(),
                                     -1,  // -1 means go until NULL char
                                     SQLITE_STATIC);
  ...
  sqliteCode = sqlite3_bind_int(m_putStatement,
                                2,  // Index of wildcard
                                fileKey);
  ...
  sqliteCode = sqlite3_bind_text(m_putStatement,
                                 3,  // Index of wildcard
                                 fileName.c_str(),
                                 -1,  // -1 means go until NULL char
                                 SQLITE_STATIC);
