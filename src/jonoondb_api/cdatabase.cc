  ...
  switch (static_cast<IndexType>(type)) {
    ...
    default:
      throw InvalidArgumentException(
          "Argument type is not valid. Allowed values are {EWAH_COMPRESSED_BITMAP = 1, VECTOR = 2}.",
          __FILE__,
          __func__,
          __LINE__);
  }
...
void jonoondb_indexinfo_setindexname(indexinfo_ptr indexInfo,
                                     const char* value) {
...
void jonoondb_indexinfo_setcolumnname(indexinfo_ptr indexInfo,
                                      const char* value) {
