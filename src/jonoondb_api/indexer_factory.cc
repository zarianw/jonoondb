...
Status CreateEWAHCompressedBitmapIndexer(const IndexInfo& indexInfo, ColumnType columnType, Indexer*& indexer) {
  switch (columnType)
  {
...
Status IndexerFactory::CreateIndexer(const IndexInfo& indexInfo,
  ...
  case IndexType::EWAHCompressedBitmap: {
    ...
    if (it == columnTypes.end()) {
      ostringstream ss;
      ss << "The column type for " << indexInfo.GetColumn(0) << " could not be determined.";
