...
DatabaseImpl::DatabaseImpl(unique_ptr<DatabaseMetadataManager> databaseMetadataManager)
...
Status DatabaseImpl::Open(const char* dbPath, const char* dbName,
  ...
  Status status = DatabaseMetadataManager::Open(dbPath, dbName, options.GetCreateDBIfMissing(),
    databaseMetadataManagerPtr);
  if (!status.OK()) {
    return status;
  }
  unique_ptr<DatabaseMetadataManager> databaseMetadataManager(databaseMetadataManagerPtr);
...
Status DatabaseImpl::CreateCollection(const char* name, int schemaType, const char* schema,
  ...
  status = m_dbMetadataMgrImpl->AddCollection(name, schemaType, schema, indexes, indexesLength);
  if (!status.OK()) {
    return status;
  }
  m_collectionContainer.insert(
    make_pair<string, unique_ptr<DocumentCollection>>(string(name), move(documentCollection)));  

  return Status();
}

Status DatabaseImpl::Insert(const char* collectionName, const Buffer& documentData) {
