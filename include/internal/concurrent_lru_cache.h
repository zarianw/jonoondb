...
namespace jonoondb_api
{
		...
		private:
			...
			public:
				...
				LRUCacheEntry(const std::shared_ptr<U1>& value, int64_t lastUsed, bool evictable) : Value(value), LastUsed(lastUsed), Evictable(evictable)
				{
				}				
			};
        ...
        boost::unique_lock<boost::shared_mutex> lock(m_mutex);
				...
				m_map[key] = std::unique_ptr<LRUCacheEntry<T2>>(new LRUCacheEntry<T2>(value, m_lastUsed, evictable));
			}
          ...
          boost::shared_lock<boost::shared_mutex> lock(m_mutex);
					...
					if(itemsToEvictCount > 0)
					{
						for(auto iter = m_map.begin(); iter != m_map.end(); iter++)
						{
							...
							for(auto keysToEvictIter = keysToEvict.begin(); keysToEvictIter != keysToEvict.end(); keysToEvictIter++)
							{
								if(iter->second->Evictable && iter->second->LastUsed < keysToEvictIter->first)
								{
									keysToEvict.insert(keysToEvictIter, std::pair<int64_t, T1>(iter->second->LastUsed, iter->first));
									itemInserted = true;
									break;
								}
							...
							if(!itemInserted && iter->second->Evictable && keysToEvict.size() < itemsToEvictCount)
							{
								keysToEvict.push_back(std::pair<int64_t, T1>(iter->second->LastUsed, iter->first));
							}
