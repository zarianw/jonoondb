...
namespace jonoondb_api
{
  ...
  {
  public:
    static Status Open(const char* dbPath, const char* dbName, bool createDBIfMissing,
                       DatabaseMetadataManager*& databaseMetadataManager);

    Status AddCollection(const char* name, int schemaType, const char* schema, const IndexInfo indexes[],
                         int indexesLength);

    ~DatabaseMetadataManager();
  private:
    DatabaseMetadataManager(const char* dbPath, const char* dbName, bool createDBIfMissing);
    Status Initialize();
    Status CreateTables();
